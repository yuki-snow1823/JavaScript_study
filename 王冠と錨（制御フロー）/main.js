...
// 理解、上と一緒に使うことでランダムの手ができる

let 残り資金 = 50;     // 開始時の条件
let ラウンド = 0;
// do {
  while (残り資金 > 0 && 残り資金 < 100) {　// この間繰り替えす
    ラウンド++;
    ...
    let 今回の賭け金の合計 = ランダムに整数を返す(1, 残り資金); // ヘルパーメソッドの呼び出し
    if (今回の賭け金の合計 === 7) {
      今回の賭け金の合計 = 残り資金; // 全てをかける
      各マスの賭け金.ハート = 今回の賭け金の合計; //実際におく
    } else {
      // 賭け金を振り分ける 何かアクション話。振り分けているだけ。
      let 残りの賭け金 = 今回の賭け金の合計;
      do {
        let このマスの賭け金 = ランダムに整数を返す(1, 残りの賭け金);　// 第一引数の１は１以上のという意味
        let 出たマス目 = ランダムにマス目を返す();
        各マスの賭け金[出たマス目] = 各マスの賭け金[出たマス目] + このマスの賭け金;//実際に主人公が置くおかね
    ...
    console.log(`  出目: ${出目.join(', ')}`);//joinは配列を新たな文字で区切って一個の文字列にする
